const droppableArea: HTMLElement = document.querySelector(
  ".drop-zone"
) as HTMLElement;
...
class ToyMove {
  ...
  constructor(img: HTMLImageElement) {
    ...
    this.toyCounter = this.toyContainer.querySelector(
      ".toy-counter"
    ) as HTMLDivElement;
  }
  ...
  public moveToy() {
    ...
    const toyClone: HTMLImageElement = this.toyImg.cloneNode(
      true
    ) as HTMLImageElement;
    ...
    toyClone.addEventListener("mousedown", (event) => {
      function moveAt(pageX: number, pageY: number) {
        toyClone.style.left = `${
          ((pageX - toyClone.offsetWidth / 2) / window.innerWidth) * 100
        }%`;
        toyClone.style.top = `${
          ((pageY - toyClone.offsetHeight / 2) / window.innerHeight) * 100
        }%`;
      }
      ...
      toyClone.onmouseup = function placeToy() {
        ...
        if (droppableFlag) {
          ...
          toyCounter.textContent = (
            toyContainer.children.length - 2
          ).toString();
        }
      };
    });
