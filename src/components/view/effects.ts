...
class Effects {
  ...
  constructor() {
    this.snowFlag =
      JSON.parse(localStorage.getItem("snowFlag") as string) || false;
    this.audioFlag =
      JSON.parse(localStorage.getItem("audioFlag") as string) || false;
    this.lightsFlag =
      JSON.parse(localStorage.getItem("lightsFlag") as string) || false;
    this.audio = new Audio("audio/audio.mp3");
  }

  private toggleSnow(): void {
    localStorage.setItem("snowFlag", JSON.stringify(!this.snowFlag));
    (document.getElementById("snow-container") as HTMLElement).classList.toggle(
      "hide"
    );
    (document.getElementById("snow-button") as HTMLElement).classList.toggle(
      "clicked"
    );
  }

  private toggleMusic(): void {
    (document.getElementById("music-button") as HTMLElement).classList.toggle(
      "clicked"
    );
  ...
  private initMusicButton(): void {
    (document.getElementById("music-button") as HTMLElement).addEventListener(
      "click",
      () => {
        this.toggleMusic();
      }
  ...
  private initSnowButton(): void {
    (document.getElementById("snow-button") as HTMLElement).addEventListener(
      "click",
      () => {
        this.toggleSnow();
      }
  ...
  private initDefaultButton(): void {
    (document.getElementById("default-tree") as HTMLElement).addEventListener(
      "click",
      () => {
        this.setDefaultSetings();
      }
    );
  }
  ...
  private setSavedSettings() {
    if (this.audioFlag) {
      (document.getElementById("music-button") as HTMLElement).classList.add(
        "clicked"
      );
    ...
    if (this.snowFlag) {
      ...
      (document.getElementById("snow-button") as HTMLElement).classList.add(
        "clicked"
      );
    }
    if (this.lightsFlag) {
  ...
  private setDefaultSetings() {
    ...
    (document.getElementById("music-button") as HTMLElement).classList.remove(
      "clicked"
    );
    (document.getElementById("snow-container") as HTMLElement).classList.add(
      "hide"
    );
    (document.getElementById("snow-button") as HTMLElement).classList.remove(
      "clicked"
    );
    this.audio.pause();
    this.audio.currentTime = 0;
    ...
    ).style.backgroundImage = 'url("tree/1.png")';
  }
  ...
  private static createSingleLight(color: string): HTMLDivElement {
    ...
    if (color === "multi") {
      ...
      light.style.animationName = `flicker${
        tempColor.charAt(0).toUpperCase() + tempColor.slice(1)
      }`;
    } else {
  ...
  private static createLights(color: string) {
    Effects.removeLights();
    document
      .querySelector(".garland-button")
      ?.addEventListener("click", Effects.removeLights);
